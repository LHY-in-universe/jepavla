...
class RTCExecutor:
    ...
    def _merge_with_leftover(self, new_chunk: Tensor) -> Tensor:
    ...
    @torch.no_grad()
    def step(self, batch: dict, num_flow_steps: Optional[int] = None) -> tuple[Tensor, Tensor]:
        chunk = self.policy.predict_action_chunk(batch, num_flow_steps=num_flow_steps)
        chunk = self._merge_with_leftover(chunk)
